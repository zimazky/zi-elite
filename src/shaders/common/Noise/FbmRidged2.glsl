...
vec4 terrainFbm(vec2 p, float dist) {
  ...
  for(; i<5; i++ ) {
    // низкие частоты
    f = noised(m*p);
    f = -abs_d(2.*f - ONE_D); // приведение к диапазону [-1 ... +1] и вычисление ridged шума
  ...
  for(; i<9; i++ ) {
    // низкие частоты
    f = noised(m*p);
    f = -(1./(1.+a*a))*abs_d(2.*f - ONE_D); // приведение к диапазону [-1 ... +1] и вычисление ridged шума
    a += b*f.w;               // накопление значения высоты
    d += f.xy * m;            // накопление величин производных (b*fr = 1.0 поэтому производные не масштабируются)
    b *= 0.5;                 // уменьшение амплитуды следующей октавы
    p *= 2.0;                 // увеличение частоты следующей октавы
  ...
  for(; i<int(noct); i++ ) {
    // высокие частоты
    f = noised(m*p);
    f = (1./(1.+a*a))*square_d(f);          // сглаживание шума на высоких частотах
    a += b*f.w;               // накопление значения высоты
